...
import {applyWindowFunction, WindowFunctionType} from "../Visual/WindowFunctions";
...
export class FFTProcessor {
  ...
  private initialize() {
    try {
      ...
      this.fftInterleavedInputBuffer = new Float32Array(
        this.options.fftSamples * 2
      );
    } catch (error) {
  ...
  updateParameters(newOptions: Partial<FFTProcessorOptions>) {
    const needsReinitialization =
      newOptions.fftSamples &&
      newOptions.fftSamples !== this.options.fftSamples;
    // Check if the sampleRate changed, as it affects MelBanks
    const needsMelCacheClear =
      newOptions.sampleRate &&
      newOptions.sampleRate !== this.options.sampleRate;
  ...
  calculatePowerSpectrum(buffer: Float32Array): Float32Array | null {
    if (
      !this.webfftInstance ||
      !this.fftInputBuffer ||
      !this.fftInterleavedInputBuffer ||
      buffer.length === 0
    ) {
  ...
  convertToMelScale(
    ...
    if (!this.melBanksCache || this.melBanksCacheKey !== currentKey) {
      try {
        this.melBanksCache = new MelBanks(
