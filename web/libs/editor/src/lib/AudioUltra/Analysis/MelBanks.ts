...
export class MelBanks {
  ...
  applyFilterbank(linearSpectrum: Float32Array): Float32Array {
    ...
    if (
      this.filterbank.length > 0 &&
      this.filterbank[0].length !== linearSpectrum.length
    ) {
      console.error(
  ...
  private createMelFilterbank(
    ...
    const binFreqs: Float32Array = new Float32Array(linearBinCount).map(
    ...
    for (let i = 0; i < numBands; i++) {
      ...
      if (leftDelta <= 0 || rightDelta <= 0) {
        ...
        const clampedBinIndex = Math.max(
          0,
          Math.min(linearBinCount - 1, targetBinIndex)
        );
