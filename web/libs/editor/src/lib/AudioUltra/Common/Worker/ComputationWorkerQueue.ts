// web/libs/editor/src/lib/AudioUltra/Common/Worker/ComputationWorkerQueue.ts

import {BatchProgress, TaskPriority} from "./ComputationQueue";
...
export class ComputationWorkerQueue {
  ...
  constructor(
    name: string,
    chunkSize: number = DEFAULT_WORKER_CHUNK_SIZE,
    priority?: TaskPriority
  ) {
    // Removed: super();
    this.name = name;
  ...
  async processChunk(): Promise<ChunkProcessingResult> {
    ...
    while (tasksProcessedInChunk < this.chunkSize && this.hasPendingTasks()) {
      ...
      try {
        ...
        console.error(
          `[WorkerQueue ${this.name}] Task ${taskToStart.id} (batch ${currentBatch.id}) failed:`,
          error
        );
      }
  ...
  get pendingTaskCount(): number {
    ...
    return this.pendingBatches.reduce(
      (sum, batch) => sum + batch.pendingTasks.length,
      0
    );
  }
  ...
  public cancelBatchesByPriority(priorities: TaskPriority[]) {
    this.pendingBatches = this.pendingBatches.filter(
      batch => !priorities.includes(batch.metadata?.priority)
    );
  }
  ...
  public getAllBatchProgresses(priority: TaskPriority): BatchProgress[] {
    return this.pendingBatches.map(batch => ({
      ...
      percentage:
        batch.totalTasks > 0
          ? Math.round((batch.processedTasks / batch.totalTasks) * 100)
          : 0
    }));
  }
