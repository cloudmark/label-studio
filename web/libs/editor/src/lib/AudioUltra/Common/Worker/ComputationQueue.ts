...
export interface PriorityProgress {
  queued: number;
  active: number;
  processed: number; // Processed since last clear for this priority
}
...
export class ComputationQueue<T = any> {
  ...
  constructor(options: ComputationQueueOptions<T> = {}) {
    ...
    const wrappedOnBatchComplete = (batchId: string, metadata: any) => {
      ...
      if (priority && this.activeBatchCounts[priority]) {
        ...
        if (
          this.activeBatchCounts[priority].size === 0 &&
          this.options.onAllCategoryComplete
        ) {
    ...
    this.highWorker = new ComputationWorkerQueue(
      "high",
      options.highBatchSize ?? 10,
      TaskPriority.HIGH
    );
    this.normalWorker = new ComputationWorkerQueue(
      "normal",
      options.normalBatchSize ?? 5,
      TaskPriority.NORMAL
    );
    this.lowWorker = new ComputationWorkerQueue(
      "low",
      options.lowBatchSize ?? 1,
      TaskPriority.LOW
    );
    ...
    if (options.onProgress) {
      const aggregateProgress = () => {
        ...
        const totalProcessed = batches.reduce(
          (sum, b) => sum + b.processedTasks,
          0
        );
        const overallPercentage =
          totalAdded > 0 ? Math.round((totalProcessed / totalAdded) * 100) : 0;
        options.onProgress!({
  ...
  public addBatch(
    tasks: { id: string; priority: TaskPriority; taskFn: () => Promise<any> }[],
    metadata: any
  ): string {
  ...
  private emitAggregateProgress() {
    if (this.options.onProgress) {
      const highBatches = this.highWorker.getAllBatchProgresses(
        TaskPriority.HIGH
      );
      const normalBatches = this.normalWorker.getAllBatchProgresses(
        TaskPriority.NORMAL
      );
  ...
  public hasTask(id: string): boolean {
    ...
    return (
      this.highWorker.hasPendingTasks() ||
      this.normalWorker.hasPendingTasks() ||
      this.lowWorker.hasPendingTasks()
    );
  }
  ...
  public getQueueSize(): number {
    return (
      this.highWorker.pendingTaskCount +
      this.normalWorker.pendingTaskCount +
      this.lowWorker.pendingTaskCount
    );
  }
