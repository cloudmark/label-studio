...
export function useSpectrogramControls(
  ...
  useEffect(() => {
    ...
    if (Object.keys(paramsToUpdate).length > 0) {
      ...
      wf.visualizer.updateSpectrogramConfig(paramsToUpdate);
    }
  }, [
  ...
  const setFftSamples = useCallback((samples: number) => {
    ...
    waveform.current.visualizer.updateSpectrogramConfig({ fftSamples: samples });
  ...
  const setMelBands = useCallback((bands: number) => {
    ...
    waveform.current.visualizer.updateSpectrogramConfig({ melBands: bands });
  ...
  const setWindowingFunction = useCallback((func: WindowFunctionType) => {
    ...
    waveform.current.visualizer.updateSpectrogramConfig({ windowingFunction: func });
  ...
  const setColorScheme = useCallback((scheme: ColorScheme) => {
    ...
    waveform.current.visualizer.updateSpectrogramConfig({ colorScheme: scheme });
  ...
  const setDbRange = useCallback((minDb: number, maxDb: number) => {
    ...
    waveform.current.visualizer.updateSpectrogramConfig({ minDb, maxDb });
    changeSetting?.("spectrogramMinDb", minDb);
    changeSetting?.("spectrogramMaxDb", maxDb);
  }, [waveform, changeSetting]);

  const setScale = useCallback((scale: SpectrogramScale) => {
      if (!waveform.current) return;
      ...
      waveform.current.visualizer.updateSpectrogramConfig({ scale });
