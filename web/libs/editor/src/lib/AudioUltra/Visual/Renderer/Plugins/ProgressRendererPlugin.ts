...
import {BatchProgress, DetailedComputationProgress, TaskPriority} from "../../../Common/Worker/ComputationQueue";
...
export class ProgressRendererPlugin implements RendererPlugin<ProgressRendererPluginConfig> {
  ...
  private overlayWidth: number = 180;
  private overlayBgColor: string = "rgba(0, 0, 0, 0.7)";
  private overlayTextColor: string;
  private overlayFontSize: number = 10;
  private barHeight: number = 12;
  private barRadius: number = 2;
  private barShadow: string = "0 1px 4px rgba(0,0,0,0.18)";
  private ariaLabel: string = "Spectrogram progress overlay";
  private labels: {
    view: string;
    precache: string;
    cached: string;
  ...
  private _createPriorityBars(batchGroups: Record<string, BatchProgress[]>): HTMLElement {
    ...
    for (const { key, color, label } of priorities) {
      ...
      let completedTasks = 0, activeTasks = 0, totalTasks = 0;
      ...
      div.appendChild(this._createBar({
