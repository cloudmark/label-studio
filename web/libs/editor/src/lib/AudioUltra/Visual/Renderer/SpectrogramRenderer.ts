...
import {ComputationQueue, DetailedComputationProgress, TaskPriority} from '../../Common/Worker/ComputationQueue';
import {
...
import {FFTProcessor, SpectrogramScale} from '../../Analysis/FFTProcessor';
...
export class SpectrogramRenderer implements Renderer<SpectrogramRendererConfig> {
  ...
  public spectrogramDrawing: boolean = false;
  public forceCachedExcedLimit: number = 0;
  ...
  public spectrogramNeedsRedraw: boolean = false;
  ...
  private spectrogramHighCompleteDebounceTime: number = 100;
  ...
  public lastSpectrogramRenderedWidth: number = 0;
  public lastSpectrogramRenderedZoom: number = 0;
  public lastSpectrogramRenderedScrollLeftPx: number = 0;
  ...
  constructor(progressContainer: HTMLElement, spectrogram: Layer,  gridLayer: Layer, config: SpectrogramRendererConfig, onRenderTransfer?: () => void) {
    this.config = config;
    ...
    this.computationQueue = new ComputationQueue<void>({
      ...
      onBatchComplete: (batchId, metadata) => {
        if (!metadata) return;
        if (metadata.type === 'current-view-partial' && metadata.startSample !== undefined && metadata.endSample !== undefined) {
    ...
    this.gridRendererPlugin = new GridRendererPlugin(gridLayer, this.colorMapper, {
      visible: this.spectrogram.isVisible, fontSize: 11, height: config.channelHeight, spectrogramScale: this.config.spectrogramScale
    });
  ...
  draw(context: RenderContext): void {
    if (this.isDestroyed || !this.spectrogram?.isVisible || !this.audio || !this.fftProcessor || !this.spectrogram.isVisible) return;
    this.lastRenderContext = context;
    ...
    try {
      ...
      const cacheLimitExceeded = this.computationQueue.getQueueSizes().high > context.width * SPECTROGRAM_CACHE_CLEAR_FACTOR;
      ...
      if (needsFullRender) {
        ...
        const visibleEndSample = Math.ceil(clamp(visibleStartSample + context.width * context.samplesPerPx, 0, dataLength));
        ...
        const tasksScheduled = this._scheduleSpectrogramTasks(
          highStart, highEnd, TaskPriority.HIGH, hopSize, fftSize, dataLength, 'current-view', SPECTROGRAM_HIGH_BATCH_SIZE
        );
        if (tasksScheduled === 0) {
        ...
        if (PRECACHE) {
          ...
          const normalBufferSamples = Math.max(visibleWindowSamples, Math.round(SPECTROGRAM_BUFFER_NORMAL_SEC * sampleRate));
          const normalStart = highEnd;
          const normalEnd = normalStart + normalBufferSamples;
          this._scheduleSpectrogramTasks(
            normalStart, normalEnd, TaskPriority.NORMAL, hopSize, fftSize, dataLength, 'precache-view-normal', SPECTROGRAM_NORMAL_BATCH_SIZE
          );
        }
  ...
  renderSpectrogramSlice(
    ...
    if (!this.lastRenderContext || !this.audio || channelHeight <= 0 || this.lastRenderContext.samplesPerPx <= 0 || !this.fftProcessor) {
      return;
    }
  ...
  private _scheduleSpectrogramTasks(
    ...
    if (tasks.length > 0) {
      ...
      const meta: any = {type: metaType, priority, startSample: intStartSample, endSample: intEndSample, correlationId};
  ...
  public redrawSpectrogramSliceFromCache(correlationId: string | undefined, startSample: number, endSample: number) {
    ...
    try {
      ...
      for (let channelIndex = 0; channelIndex < this.audio!.channelCount; channelIndex++) {
        this.renderSpectrogramSlice(this.spectrogram, this.config.channelHeight, clampedStart, clampedEnd, channelIndex, startX);
      }
    } catch (error) {
  ...
  private getChannelDataSlice(channelIndex: number, startSample: number, endSample: number): Float32Array | null {
    if (!this.audio || !this.audio.chunks || !this.audio.chunks[channelIndex] || startSample >= endSample || channelIndex < 0 || channelIndex >= this.audio.channelCount) {
      return null;
    }
    ...
    for (const sourceChunk of sourceChunks) {
      ...
      if (overlapStart < overlapEnd) {
        ...
        if (sourceStartIndex >= 0 && sourceStartIndex + copyLength <= sourceChunk.length &&
          outputStartIndex >= 0 && outputStartIndex + copyLength <= outputBuffer.length) {
